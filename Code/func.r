...
Calu.SingleVir.plot <- function(dat, target, save, path, width = 16, height = 8, ...) {
  ...
  fig <- ggplot(FigDat) +
    ...
    theme(
      ...
      axis.title.x = element_text(size = 20, face = "bold", colour = "black", vjust = -1),
      axis.title.y = element_text(size = 20, face = "bold", colour = "black", vjust = 2.5),
      strip.text = element_text(size = 18, face = "bold", colour = "black"),
      strip.background = element_rect(colour = "grey20", fill = "white", linewidth = 0.6),
...
Calu.TwoVirInterval <- function(dat, Wave, NPIRef = c("last", "refday"), NPIdat = NULL) {
  dat_modify <- dat[Index_of_Wave == Wave]
...
Liner.cal <- function(dat, target) {
  if (length(dat$FirstVir) > length(dat$Country)) {
    EXPRESS <- as.formula(paste0(target, "~ 1 + (1 | Country)"))
    model <- lmer(EXPRESS, data = dat)
  } else {
  ...
  Table <- data.frame(
    ...
    SecVir = dat$SecVir[1],
    num = dat$num[1],
    mean = Summ[["coefficients"]][[1]],
    se = Summ[["coefficients"]][[2]],
    lci = Summ[["coefficients"]][[1]] - 1.96 * Summ[["coefficients"]][[2]],
    uci = Summ[["coefficients"]][[1]] + 1.96 * Summ[["coefficients"]][[2]]
  )
...
ReportTwoVir <- function(dat, index = c("peak", "last"),
  ...
  Report2Vir <- do.call(rbind, lapply(Report2Vir_split, Liner.cal, target = target))
  Report2Vir <- Report2Vir[order(-Report2Vir$mean), ]
  Report2Vir <- na.omit(Report2Vir)
...
Matching.Plot <- function(
    dat,
    cex.labels = 3,
    font.labels = 2) { # Font: 1 plain, 2 bold, 3 italic, 4 bold italic
  Num_col <- length(unique(dat$FirstVir)) + 1
  UpperLimit <- max(dat[, 3:ncol(dat)], na.rm = TRUE) + 50
  LowerLimit <- min(dat[, 3:ncol(dat)], na.rm = TRUE) - 50
  UpperLimit <- ceiling(UpperLimit / 50) * 50
  LowerLimit <- floor(LowerLimit / 50) * 50
  ...
  opar <- par(
    mfrow = c(Num_col, Num_col), # Setting matrix
  ...
  for (i in 1:Num_col) {
    for (j in 1:Num_col) {
      ...
      if (i == j) {
        ...
        text(
          x = 0.5, y = 0.5, labels = levels(dat[, 1])[i],
          cex = cex.labels, font = font.labels
        )
        box()
      } else if (i < j) {
        # Upper panel
        Panel.ErrorBar(i, j, dat, UpperLimit, LowerLimit)
      } else {
        # Lower panel
        Panel.Num(i, j, dat, font.labels)
      }
    }
  }
  Fig <- recordPlot()
...
Panel.ErrorBar <- function(i, j, dat, UpperLimit, LowerLimit) {
  ExtractDat <- dat[dat$FirstVir == levels(dat$FirstVir)[i] & dat$SecVir == levels(dat$FirstVir)[j], ]

  plot.window(xlim = c(-1, 1), ylim = c(LowerLimit, UpperLimit))
  abline(h = 0, lty = 2, col = "gray", lwd = 2)

  # error bar left
  segments(
    x0 = -0.5, y0 = ExtractDat$lci,
    x1 = -0.5, y1 = ExtractDat$uci,
    col = "#00468BFF", lwd = 2
  )
  points(x = -0.5, y = ExtractDat$mean, pch = 16, col = "#00468BFF", cex = 2)

  # number left
  # text(x=-0.7, y=ExtractDat$mean,
  #      labels=ExtractDat$mean,
  #      cex=3,
  #      col="#00468BFF",
  #      adj=1)  # align right
  ...
  # error barright
  segments(
    x0 = 0.5, y0 = ExtractDat$lci_Sec,
    x1 = 0.5, y1 = ExtractDat$uci_Sec,
    col = "#ED0000FF", lwd = 2
  )
  points(x = 0.5, y = ExtractDat$mean_Sec, pch = 16, col = "#ED0000FF", cex = 2)

  # number right
  # text(x=0.7, y=ExtractDat$mean_Sec,
  #      labels=ExtractDat$mean_Sec,
  #      cex=3,
  #      col="#ED0000FF",
  #      adj=0)  # align left
  box()
...
Panel.Num <- function(i, j, dat, font.labels) {
  ExtractDat <- dat[dat$FirstVir == levels(dat$FirstVir)[j] & dat$SecVir == levels(dat$FirstVir)[i], ]

  plot.window(xlim = c(-1, 1), ylim = c(-1, 1))
  text(-0.5, 0, cex = 3, font = font.labels, ExtractDat$mean, col = "#00468BFF") # ExtractDat$num
  text(0.5, 0, cex = 3, font = font.labels, ExtractDat$mean_Sec, col = "#ED0000FF") # ExtractDat$num_Sec
  box()
}


Merge.Sensitivity <- function(DatA, DatB) {
  GlobalColOrder <- MergeTable_Recir[, 1:2]
  NewDat <- MergeTable_Recir
  NewDat[, 3:ncol(NewDat)] <- NA
  ## Re-order the columns
  DatA$FirstVir <- factor(DatA$FirstVir, levels = levels(GlobalColOrder[, 1]))
  DatA$SecVir <- factor(DatA$SecVir, levels = levels(GlobalColOrder[, 1]))
  DatB$FirstVir <- factor(DatB$FirstVir, levels = levels(GlobalColOrder[, 1]))
  DatB$SecVir <- factor(DatB$SecVir, levels = levels(GlobalColOrder[, 1]))

  for (i in 1:max(nrow(DatA), nrow(DatB))) {
    FirstVir_A <- DatA[i, "FirstVir"]
    SecVir_A <- DatA[i, "SecVir"]
    FirstVir_B <- DatB[i, "FirstVir"]
    SecVir_B <- DatB[i, "SecVir"]

    DirectMatch_A <- which(NewDat$FirstVir == FirstVir_A & NewDat$SecVir == SecVir_A)
    ReverseMatch_A <- which(NewDat$FirstVir == SecVir_A & NewDat$SecVir == FirstVir_A)
    ...
    DirectMatch_B <- which(NewDat$FirstVir == FirstVir_B & NewDat$SecVir == SecVir_B)
    ReverseMatch_B <- which(NewDat$FirstVir == SecVir_B & NewDat$SecVir == FirstVir_B)
    ...
    if (length(DirectMatch_A) > 0) {
      NewDat[NewDat$FirstVir == FirstVir_A & NewDat$SecVir == SecVir_A, 3:7] <-
        DatA[DatA$FirstVir == FirstVir_A & DatA$SecVir == SecVir_A, 3:ncol(DatA)]
    } else if (length(ReverseMatch_A) > 0) {
      NewDat[NewDat$FirstVir == FirstVir_A & NewDat$SecVir == SecVir_A, 3:7] <-
        -DatA[DatA$FirstVir == SecVir_A & DatA$SecVir == FirstVir_A, 3:ncol(DatA)]
    }

    if (length(DirectMatch_B) > 0) {
      NewDat[NewDat$FirstVir == FirstVir_B & NewDat$SecVir == SecVir_B, 8:12] <-
        DatB[DatB$FirstVir == FirstVir_B & DatB$SecVir == SecVir_B, 3:7]
    } else if (length(ReverseMatch_B) > 0) {
      NewDat[NewDat$FirstVir == FirstVir_B & NewDat$SecVir == SecVir_B, 8:12] <-
        -DatB[DatB$FirstVir == SecVir_B & DatB$SecVir == FirstVir_B, 3:7]
    }
  }

  # NewDat$num_Sec <- abs(NewDat$num_Sec)
  # NewDat$num <- abs(NewDat$num)
