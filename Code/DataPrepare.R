...
FileName <- "Data/Full-text table-V1.6.xlsx"

Income <- read_excel("Data/income.xlsx") # World bank income classification

General <- read_excel(FileName, sheet = "General") %>% left_join(., Income, by = join_by(Country)) %>% rowwise() %>% 
  mutate(across(starts_with("Q"), as.logical),
         quality = sum(c_across(Q1:Q8), na.rm = T)) %>% as.data.table()
Vir_list <- read_excel(FileName, sheet = "Vir-list", col_types = c("text", "text", "text", "text", "text", "text", "text", "text",
                                                                   "text", "numeric", "date", "date", "numeric", "text", "date",
                                                                   "date", "numeric", "text", "text", "numeric")) %>% as.data.table()
...
All_Vir <- right_join(General, Vir_list,  multiple = "all") %>%
  ...
  mutate(Virus_ID = case_when(Virus_name == 'IFV' ~ 'V1',
                               Virus_name == 'RSV' ~ 'V2',
                               Virus_name == 'HPIV' ~ 'V3',
                               Virus_name == 'HMPV' ~ 'V4',
                               Virus_name == 'HCoV' ~ 'V5',
...
# save.image("~/Documents/600_Project/610_System_review/Location.RData")
...
All_Vir <- All_Vir %>% 
  mutate(Virus_name = case_when(Virus_name == 'IFV' ~ 'IFV',
                                Virus_name == 'RSV' ~ 'RSV',
                                Virus_name == 'HPIV' ~ 'PIV',
                                Virus_name == 'HMPV' ~ 'MPV',
                                ...
                                Virus_name == 'HRV' ~ 'RV',
                                Virus_name == 'HAdV' ~ 'AdV',
                                Virus_name == 'IFVA' ~ 'IFVA',
                                Virus_name == 'IFVB' ~ 'IFVB'))


rm(list = c(ls()[!ls() %in% c("All_Vir", "General", "Vir_list", "LocationInfo")]))
save.image("~/Documents/600_Project/610_System_review/All_Vir.RData")
