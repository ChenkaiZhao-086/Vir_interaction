...
FilePath.Sens1 <- CreateSubFolder(FilePath, "4.IFVA")
...
for (i in 1:9) {
  for (j in 1:9) {
    ...
    if (i == j) {
      ...
      # Upper panel
      Panel.ErrorBar(i, j, Sens1_MergeTable_Recir, 1000, -200)
    } else {
      # Lower panel
...
colnames(Sens1_MergeTable_Peak) <- c("FirstVir", "SecVir", "num", "mean", "se", "lci", "uci", 
                                     "num_Sec", "mean_Sec", "se_Sec", "lci_Sec", "uci_Sec")
## Re-order the columns
...
for (i in 1:9) {
  for (j in 1:9) {
    ...
    if (i == j) {
      ...
      # Upper panel
      Panel.ErrorBar(i, j, Sens1_MergeTable_Peak, 1000, -200)
    } else {
      # Lower panel
