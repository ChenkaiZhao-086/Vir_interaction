...
FilePath.Sens1 <- CreateSubFolder(FilePath, "2.Tropics")

SensDat1 <- MainDat[lat <= 23.5 & lat >= -23.5]
...
MergedDat <- merge(ColIndex, Sens1_MainAnalysis_Recir_REM, all.x = T)
Calu.SingleVir.plot(MergedDat, target = "Time_interval", save = T, path = paste0(FilePath.Sens1, "MainAnalysis_Recir.pdf"), width = 16, height = 20)
